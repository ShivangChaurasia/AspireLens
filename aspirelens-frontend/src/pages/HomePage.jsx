...
export default function HomePage() {
    ...
    const [isVisible, setIsVisible] = useState({
        badge: false, title: false, subtitle: false,
        button: false, features: false, stats: false
    });
    ...
    // Mouse movement + IntersectionObserver
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            (entries) => entries.forEach(entry => {
                ...
                if (entry.isIntersecting) setIsVisible(prev => ({ ...prev, [id]: true }));
            }),
    ...
    // Canvas background animation (landing section)
    useEffect(() => {
        ...
        const drawGrid = () => {
            ...
            ctx.lineWidth = 1;
            const g = 50;
            const ox = mousePosition.x * 0.1, oy = mousePosition.y * 0.1;
            for (let x = ox % g; x < canvas.width; x += g) {
                ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke();
            }
            for (let y = oy % g; y < canvas.height; y += g) {
        ...
        const drawParticles = () => {
            particles.forEach(p => {
                ...
                p.x = (p.x + orbitX * 0.01 + time * p.speedX + canvas.width) % canvas.width;
                p.y = (p.y + orbitY * 0.01 + time * p.speedY + canvas.height) % canvas.height;
                ctx.beginPath();
                ...
                particles.forEach(other => {
                    ...
                    if (dist < 100) {
                        ...
                        ctx.lineWidth = 0.5;
                        ctx.moveTo(p.x, p.y); ctx.lineTo(other.x, other.y); ctx.stroke();
                    }
                });
            });
        };
        ...
        const drawBlobs = () => {
            const g1 = ctx.createRadialGradient(
                canvas.width * 0.3 + mousePosition.x * 0.2, canvas.height * 0.3 + mousePosition.y * 0.2, 0,
                canvas.width * 0.3 + mousePosition.x * 0.2, canvas.height * 0.3 + mousePosition.y * 0.2, 300
            );
            g1.addColorStop(0, 'rgba(59,130,246,0.1)'); g1.addColorStop(1, 'rgba(59,130,246,0)');
            const g2 = ctx.createRadialGradient(
                canvas.width * 0.7 - mousePosition.x * 0.2, canvas.height * 0.7 - mousePosition.y * 0.2, 0,
                canvas.width * 0.7 - mousePosition.x * 0.2, canvas.height * 0.7 - mousePosition.y * 0.2, 400
            );
            g2.addColorStop(0, 'rgba(6,182,212,0.1)'); g2.addColorStop(1, 'rgba(6,182,212,0)');
        ...
        const animate = () => {
            ...
            drawBlobs(); drawGrid(); drawParticles();
            time += 0.01;
    ...
    const features = [
        { icon: "🧭", title: "Smart Navigation", desc: "Intuitive mapping with AI-powered insights", gradient: "from-blue-500 to-cyan-500" },
        { icon: "⚡", title: "Real-Time Analysis", desc: "Process information instantly for faster decisions", gradient: "from-amber-500 to-orange-500" },
        { icon: "🎯", title: "Purpose-Driven", desc: "Tools designed with your goals in mind", gradient: "from-cyan-500 to-teal-500" }
    ];
    ...
    return (
        <div ref={containerRef}>
                ...
                <section className="min-h-screen flex flex-col items-center justify-center px-6 md:px-20 py-20 bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 transition-colors duration-300">
                    <div className="max-w-6xl mx-auto text-center">
                        ...
                        <p className="text-xl md:text-2xl text-gray-700 dark:text-gray-300 max-w-3xl mx-auto mb-10 leading-relaxed">
                            ...
                            <span className="italic text-gray-600 dark:text-gray-400 mt-2 block">Let's build your future, one smart step at a time.</span>
                        </p>
                        ...
                        {/* Feature cards for logged-in */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-blue-100 dark:border-gray-700 transition-colors duration-300">
                                <div className="text-3xl text-blue-600 mb-3">📊</div>
                                <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-2">AI Insights Dashboard</h3>
                                <p className="text-gray-600 dark:text-gray-400">Track your goals, test results, strengths, and growth patterns in one place.</p>
                            </div>
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-cyan-100 dark:border-gray-700 transition-colors duration-300">
                                <div className="text-3xl text-cyan-600 mb-3">🎯</div>
                                <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-2">Personalized Roadmap</h3>
                                <p className="text-gray-600 dark:text-gray-400">Get a step-by-step AI-generated plan aligned with your dream career.</p>
                            </div>
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-purple-100 dark:border-gray-700 transition-colors duration-300">
                                <div className="text-3xl text-purple-600 mb-3">🚀</div>
                                <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-2">Skill Progress Tracker</h3>
                                <p className="text-gray-600 dark:text-gray-400">Stay consistent with habit tracking, goal streaks, and weekly progress logs.</p>
                            </div>
                        </div>
                        ...
                        <div className="mt-16 pt-10 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div className="text-blue-700 dark:text-blue-400 font-semibold opacity-70">
            ...
            <section className="relative min-h-screen flex flex-col items-center justify-center px-6 md:px-12 lg:px-20 py-20 bg-gray-900 overflow-hidden">
                {/* Canvas background */}
                ...
                <div className="absolute inset-0 overflow-hidden">
                    <div className="absolute w-96 h-96 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 rounded-full blur-3xl"
                        style={{ top: '20%', left: '10%', transform: `translate(${mousePosition.x * 0.1}px, ${mousePosition.y * 0.1}px)` }} />
                    <div className="absolute w-80 h-80 bg-gradient-to-r from-amber-500/5 to-orange-500/5 rounded-full blur-3xl"
                        style={{ top: '60%', right: '15%', transform: `translate(${-mousePosition.x * 0.15}px, ${-mousePosition.y * 0.15}px)` }} />
                </div>
                ...
                <div className="relative z-10 max-w-7xl mx-auto w-full">
                    ...
                    {/* Badge */}
                    <div data-section="badge" className={`transition-all duration-1000 ${isVisible.badge ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 backdrop-blur-sm rounded-full border border-blue-500/20 mb-8 hover:border-blue-500/40 hover:scale-105 transition-all duration-300">
                            <div className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse" />
                            <span className="text-cyan-300 text-sm font-semibold tracking-wide">Intelligent Exploration Platform</span>
                            <div className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse" />
                        </div>
                    </div>

                    {/* Title */}
                    <div data-section="title" className={`mb-8 transition-all duration-1000 ${isVisible.title ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    ...
                    {/* Subtitle */}
                    <div data-section="subtitle" className={`mb-12 transition-all duration-1000 delay-300 ${isVisible.subtitle ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                            ...
                            <div className="relative inline-block group">
                                <button className="relative px-8 py-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-bold text-lg rounded-xl overflow-hidden transform group-hover:scale-105 group-hover:shadow-[0_0_40px_rgba(6,182,212,0.5)] transition-all duration-500">
                                    ...
                                    <span className="relative z-10 flex items-center gap-3">
                                        ...
                                        {user ? "Go to Dashboard" : "Evaluate Your Career"}
                    ...
                    {/* Features grid */}
                    <div data-section="features" className={`grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto mb-20 transition-all duration-1000 delay-700 ${isVisible.features ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
                        {features.map((feature, index) => (
                            <div key={feature.title} className="relative group" style={{ transitionDelay: `${index * 100}ms` }}>
                                ...
                                <div className="relative bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl border border-gray-700/50 group-hover:border-gray-600/50 transition-all duration-300 h-full">
                                    <div className="relative mb-6">
                                        <div className="text-4xl transform group-hover:scale-110 group-hover:rotate-12 transition-all duration-500">{feature.icon}</div>
                    ...
                    {/* Stats */}
                    <div data-section="stats" className={`grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto transition-all duration-1000 delay-900 ${isVisible.stats ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
                        {[
                            ...
                            <div key={stat.label} className="text-center group" style={{ transitionDelay: `${index * 100}ms` }}>
                    ...
                    <div className="mt-20 pt-12 border-t border-gray-800/50">
                        <p className="text-gray-400 text-sm text-center mb-8">Trusted by forward-thinking individuals and teams</p>
                        <div className="flex flex-wrap justify-center items-center gap-6 md:gap-12">
                            {["🚀 Explorers", "🔬 Researchers", "🎓 Educators", "📊 Analysts", "🎨 Creators"].map((item, index) => (
                                <div
                                    key={item}
                                    className="text-gray-300 font-medium px-4 py-2 bg-gray-800/30 backdrop-blur-sm rounded-lg border border-gray-700/30 hover:border-cyan-500/30 hover:text-cyan-300 hover:scale-105 hover:shadow-lg transition-all duration-300"
                                    style={{ animation: `logo-float ${3 + index}s infinite ease-in-out`, animationDelay: `${index * 0.2}s` }}
                                >
                                    {item}
                                </div>
                            ))}
