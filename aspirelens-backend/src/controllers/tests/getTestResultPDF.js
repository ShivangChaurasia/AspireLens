...
export const getTestResultPDF = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="AspireLens_Result_${testSessionId}.pdf"`);
    ...
    doc.fillColor('#374151')
       .fontSize(10)
       .text(`Name: ${user?.name || 'N/A'}`, 50, 150)
       .text(`Email: ${user?.email || 'N/A'}`, 50, 165)
       .text(`Class Level: ${user?.classLevel || 'N/A'}`, 50, 180)
       .text(`Stream: ${user?.stream || 'N/A'}`, 50, 195);

    // Test Info
    doc.fillColor('#111827')
       .fontSize(16)
       .text('Test Information', 50, 230);
    
    doc.fillColor('#374151')
       .fontSize(10)
       .text(`Test Session: ${testSessionId}`, 50, 260)
       .text(`Level: ${testSession.level || 1}`, 50, 275)
       .text(`Submitted: ${new Date(testSession.submittedAt).toLocaleDateString()}`, 50, 290)
       .text(`Evaluated: ${new Date(testResult.evaluatedAt).toLocaleDateString()}`, 50, 305);

    // Score Summary
    doc.fillColor('#111827')
       .fontSize(16)
       .text('Score Summary', 50, 340);
    ...
    const scores = [
      ...
      ['Unattempted', testResult.totalQuestions - testResult.attemptedQuestions],
      ['Score Percentage', `${testResult.scorePercentage}%`],
      ['Accuracy', `${Math.round((testResult.correctAnswers / testResult.attemptedQuestions) * 100)}%`]
    ];
    ...
    doc.fillColor('#6B7280')
       ...
       .text('This is an AI-generated report. For detailed counselling, book a session.', 50, 765);
